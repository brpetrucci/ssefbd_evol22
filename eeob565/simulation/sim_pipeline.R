...
simulate_rep <- function(lambda0, lambda1, mu0, mu1, q01, q10, N, rho) {
  ...
  while (!bounds) {
    ...
    nSampled <- sum(!(unique(floor(as.numeric(sub("t", "", sample$Species)))) 
                      %in% which(sim$EXTANT)))
    ...
    treeUltraTraits <- 
      as.numeric(unlist(lapply(ultraTaxa, function(x)
        tail(traits[[x]]$value, 1))))
    
    names(treeUltraTraits) <- paste0("t", ultraTaxa)
    
    # and the fbd tree
    treeFBDTraits <- 
      as.numeric(unlist(lapply(fbdTaxa, function(x) {
        # get traits df
        tr <-  traits[[floor(x)]]
    ...
    names(treeFBDTraits) <- paste0("t", fbdTaxa)
    
    # check the minimum percentage of the rare trait
    ratioTrait <- min(sum(treeUltraTraits)/length(ultraTaxa),
                      sum(treeFBDTraits)/length(fbdTaxa))
...
save_sims <- function(nReps, simReps, targetDir) {
  ...
  traitsFBD <- lapply(1:nReps, function(x) simReps[[x]]$TRAITSFBD)
  ...
  invisible(lapply(1:nReps, function(x)
    write.nexus.data(traitsUltra[[x]], 
                     file = paste0(traitsUltraDir, "traits_ultra_", x, ".nex"),
                     format = "standard")))
  
  invisible(lapply(1:nReps, function(x)
    write.nexus.data(traitsFBD[[x]], 
                     file = paste0(traitsFBDDir, "traits_fbd_", x, ".nex"),
                     format = "standard")))
